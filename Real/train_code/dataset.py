...
class dataset(tud.Dataset):
    ...
    def __getitem__(self, index):
        ...
        px = random.randint(0, shape[0] - self.size_h)
        py = random.randint(0, shape[1] - self.size_w)
        label = hsi[px:px + self.size_h:1, py:py + self.size_w:1, :]
        ...
        pxm = random.randint(0, 660 - self.size_h)
        pym = random.randint(0, 660 - self.size_w)
        mask_3d = self.mask_3d[pxm:pxm + self.size_h:1, pym:pym + self.size_w:1, :]

        mask_3d_shift = np.zeros((self.size_h, self.size_w + (28 - 1) * 2, 28))
        mask_3d_shift[:, 0:self.size_w, :] = mask_3d
        for t in range(28):
        ...
        if self.isTrain == True:
            ...
            rotTimes = random.randrange(0, 3,2)
        ...
        temp = mask_3d * label
        temp_shift = np.zeros((self.size_h, self.size_w + (28 - 1) * 2, 28))
        temp_shift[:, 0:self.size_w, :] = temp
        for t in range(28):
