...
def tensorboard_show(writer, epoch, recon):

    recon_spectral = np.expand_dims(recon[0, :, :, :], 1).repeat(3, 1)
    ...
    map_RGB = np.expand_dims(map_RGB, 2).repeat(660,2)  #--20240912--
    map_RGB = np.expand_dims(map_RGB, 3).repeat(660, 3)
...
def checkpoint_load(model, optimizer, scheduler, epoch, model_path):
