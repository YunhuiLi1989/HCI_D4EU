...
class WindowExdAttention(nn.Module):
    def __init__(self, dim, window_size=(5, 5), focal_level=1, num_heads=1, topK=100):
        super().__init__()
        self.dim = dim
        self.window_size = window_size
        self.input_resolution_Wh = 220 // (dim // 28)
        self.input_resolution_Ww = 660 // (dim // 28)
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5
        self.focal_level = focal_level
        self.nWh, self.nWw = (220 // (dim // 28)) // self.window_size[0], (660 // (dim // 28)) // self.window_size[1]
        ...
        for k in range(self.focal_level):
            if k == 0:
                range_h = self.input_resolution_Wh
                range_w = self.input_resolution_Ww
            else:
                range_h = self.nWh
                range_w = self.nWw
    ...
    def forward(self, x):
        ...
        sim = (q_windows @ k_all.transpose(-2, -1))
        attn = sim.softmax(dim=-1)
...
class BlockSpeAttn(nn.Module):
    def __init__(self, dim, window_size=(5, 5), dim_head=28, heads=1, shift=False):
        super().__init__()
        self.dim = dim
        self.heads = heads
...
class SpaFFT(nn.Module):
    ...
    def forward(self, x):
        ...
        Hp = int((np.ceil(H / 2)) * 2)
        Wp = int((np.ceil(W / 2)) * 2)
        xf = torch.fft.rfft2(x, s=(Hp, Wp), dim=(1, 2), norm='ortho')
        weight = torch.view_as_complex(self.complex_weight)
        xf = xf * weight
        out = torch.fft.irfft2(xf, s=(Hp, Wp), dim=(1, 2), norm='ortho')
        if Hp != H:
            out = out[:, :-1, :-1, :]
...
class SpeDCT(nn.Module):
    def __init__(self, dim):
        super().__init__()
        if dim == 28:
            self.weight = nn.Parameter(torch.randn(220, 660, dim, dtype=torch.float32) * 0.02)
        if dim == 56:
            self.weight = nn.Parameter(torch.randn(110, 330, dim, dtype=torch.float32) * 0.02)
        if dim == 112:
            self.weight = nn.Parameter(torch.randn(55, 165, dim, dtype=torch.float32) * 0.02)
...
class PSA_p(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=1, stride=1):
        ...
        self.conv_q_left = nn.Conv2d(self.inplanes, self.inter_planes * 4, kernel_size=1, stride=stride, padding=0, bias=False)
...
class SSRU(nn.Module):
    def __init__(self):
        ...
        self.down1 = SSRB(dim=28, window_size=(5, 5), dim_head=28, heads=1)
        self.downsample1 = nn.Conv2d(28, 56, 4, 2, 1, bias=False)
        self.down2 = SSRB(dim=56, window_size=(5, 5), dim_head=28, heads=2)
        self.downsample2 = nn.Conv2d(56, 112, 4, 2, 1, bias=False)
        self.bottleneck = SSRB(dim=112, window_size=(5, 5), dim_head=28, heads=4)
        self.upsample2 = nn.ConvTranspose2d(112, 56, 2, 2)
        self.fusion2 = nn.Conv2d(112, 56, 1, 1, 0, bias=False)
        self.up2 = SSRB(dim=56, window_size=(5, 5), dim_head=28, heads=2)
        self.upsample1 = nn.ConvTranspose2d(56, 28, 2, 2)
        self.fusion1 = nn.Conv2d(56, 28, 1, 1, 0, bias=False)
        self.up1 = SSRB(dim=28, window_size=(5, 5), dim_head=28, heads=1)
...
class D4EU(torch.nn.Module):
    ...
    def shift_back(self, x, len_shift=2, res=660):
        for i in range(28):
    ...
    def forward(self, g, input_mask):
        ...
        meas_s = self.shift_back(x=meas_r, res=660)
